...
public class CmdTaskFactory implements TaskFactory {
    ...
    public Task apply(Integer integer) {
        return switch (random().nextInt(4)) {
            case 0 -> Question.getAdditionQuestion(CmdQuestion::new, random(), integer);
            case 1 -> Question.getSubtractionQuestion(CmdQuestion::new, random(), integer);
            case 2 -> Question.getMultiplicationQuestion(CmdQuestion::new, random(), integer);
            case 3 -> Question.getDivisionQuestion(CmdQuestion::new, random(), integer);
